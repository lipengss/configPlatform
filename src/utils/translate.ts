 ...
 */
export function calculateRotatedPointCoordinate(point: Pos, center: Pos, rotate: number): Pos {
	/**
	 * 旋转公式：
	 *  点a(x, y)
	 *  旋转中心c(x, y)
 ...
 */
export function getRotatedPointCoordinate(style: any, center: Pos, name: string): Pos {
...
// 求两点之间的中点坐标
export function getCenterPoint(p1: { x: number; y: number }, p2: { x: number; y: number }): Pos {
	return {
		x: p1.x + (p2.x - p1.x) / 2,
		y: p1.y + (p2.y - p1.y) / 2
	}
